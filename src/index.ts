...
let guessed = false;
while(1) {
    let toGuess: number = Math.floor(Math.random() * 100 + 1)
    // console.log(chalk.gray(toGuess));
    for (; round <= rounds; round++) {
        let currError: number;
        let ans: { nGuess: string } = await inquirer.prompt([{
            name: 'nGuess',
